...
export default class GameController {
    ...
    */
    ...
    loadLevel(){
        // this.levels[this.currentLevel].platforms.forEach((platform)=>{
        //     debugger
        //     var image = new Image();
    ...
    calculateSize(unit){
        return this.canvas.width/24*unit
    }
    ...
    loadData(){
        this.totalGame.forEach(function(level){
            ...
            level.platforms.forEach(function(platform){
                nivel.platforms.push(new Platform(this, platform.piece.src, this.calculateSize(platform.x), this.calculateSize(platform.y), this.calculateSize(platform.w), this.calculateSize(platform.h), platform.isDashBreakable, platform.isDownBreakable));
            }.bind(this))
           
            //collectable items
            level.collectableItems.forEach(function(collectableItem){
                nivel.collectableItems.push(new Item(this.ctx, collectableItem.piece.src, this.calculateSize(collectableItem.x), this.calculateSize(collectableItem.y), this.calculateSize(collectableItem.w), this.calculateSize(collectableItem.h), collectableItem.isActive))
            }.bind(this))

             //damage items
            if(level.damageItems){
                level.damageItems.forEach(function(damageItem){
                    nivel.damageItems.push(new Item(this.ctx, damageItem.piece.src, this.calculateSize(damageItem.x), this.calculateSize(damageItem.y), this.calculateSize(damageItem.w), this.calculateSize(damageItem.h), damageItem.isActive, damageItem.damage))
                }.bind(this))
            }
             ...
             if(level.powerItems){
                level.powerItems.forEach(function(powerItem){
                    nivel.powerItems.push(new Item(this.ctx, powerItem.piece.src, this.calculateSize(powerItem.x), this.calculateSize(powerItem.y), this.calculateSize(powerItem.w), this.calculateSize(powerItem.h), powerItem.isActive, powerItem.damage, powerItem.type))
                }.bind(this))
            }
             ...
             if(level.backImages){
                level.backImages.forEach(function(backImage){
                    nivel.backImages.push(new Images(this.ctx, this.calculateSize(backImage.x), this.calculateSize(backImage.y), this.calculateSize(backImage.w), this.calculateSize(backImage.h), backImage.piece.src))
                }.bind(this))
            }
            ...
            if(level.frontImages){
                level.frontImages.forEach(function(frontImage){
                    nivel.frontImages.push(new Images(this.ctx, this.calculateSize(frontImage.x), this.calculateSize(frontImage.y), this.calculateSize(frontImage.w), this.calculateSize(frontImage.h), frontImage.piece.src))
                }.bind(this))
            }
